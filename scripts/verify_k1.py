...
ONE forward Vertex-Block-Descent sweep on a 10x10 mass-spring grid.
Compares dL/dp four ways. p is the handle's x-force. L is probe-node
matching, 0.5||x_probe - rest_probe||^2.

Local 2x2 block adjoint (the reverse local solve).
Paper (3D): each vertex i holds a 3x3 SPD block H_i = dg_i/dx_i.
This 2D slice uses the same object with a 2x2 block (z omitted).
"""
...
HANDLE = idx(3, 1)
PROBE = idx(4, 1)
P_INDEX = 2 * HANDLE  # dL/d(handle force_x)
...
def main() -> int:
    ...
    print("K=1 verification  *  10x10 VBD mass-spring  *  ONE sweep")
    print("p = handle force_x    L = 0.5||x_probe - rest||^2")
    print()
    print(f"{'method':<24} {'dL/dp':>14} {'vs FD':>10}  memory")
    print("-" * 72)
